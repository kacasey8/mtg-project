...
fmt_labeled_default = {
    ...
    field_flavor : field_label_flavor
}
...
class Card:
    ...
    def _set_flavor(self, values):
        ...
        for idx, value in values:
            if first:
                first = False
                self.__dict__[field_flavor] = value
            else:
                self.valid = False
                self.__dict__[field_other] += [(idx, '<text> ' + str(value))]
    ...
    def format(self, gatherer = False, for_forum = False, vdump = False, for_html = False):
        ...
        if gatherer:
            ...
            if field_flavor in self.__dict__ and self.__dict__[field_flavor]:
                outstr += linebreak
                outstr += self.__dict__[field_flavor]
